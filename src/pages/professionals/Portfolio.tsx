...
const GoldRule = () => (
  <div className="h-1 bg-gradient-to-r from-brand-yellow to-status-error" />
);

const SectionNum = ({ n, color = 'text-brand-yellow' }: { n: string; color?: string }) => (
  <div className={`text-4xl sm:text-5xl font-extrabold leading-none min-w-[44px] ${color}`}>{n}</div>
);

const Eyebrow = ({ label, color = 'text-ink-caption' }: { label: string; color?: string }) => (
...
const SectionHead = ({
  ...
  eyebrowColor = 'text-ink-caption',
  titleColor = 'text-ink',
  descColor = 'text-ink-caption',
}: {
...
const FeatCard = ({ icon, badge, title, desc, bullets, bulletColor = 'bg-brand-yellow' }: CardItem) => (
  <div className="bg-surface-card p-5">
    {icon && <span className="text-2xl block mb-2">{icon}</span>}
    {badge && <div className="mb-2">{badge}</div>}
    <h4 className="text-[13px] font-bold text-ink mb-1.5 leading-snug">{title}</h4>
    <p className="text-[11px] text-ink-caption leading-relaxed mb-3">{desc}</p>
    <ul className="border-t border-ink-light pt-2 space-y-1.5">
      {bullets.map((b, i) => (
        <li key={i} className="flex items-start gap-2 text-[11px] text-ink-paragraph leading-snug">
...
const FeatGrid = ({ children, bg = 'bg-ink-light' }: { children: React.ReactNode; bg?: string }) => (
  <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px border-2 border-ink-light rounded overflow-hidden ${bg}`}>
    {children}
  </div>
);
    ...
    <div className="pt-[104px] min-h-screen bg-surface-main font-sans">
      ...
      <section className="bg-ink text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10 items-center">
          <div>
            <div className="text-[10px] font-bold tracking-[0.18em] text-brand-yellow uppercase mb-3">Platform Profile — FY 2026-27</div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold leading-tight tracking-tight mb-2">
              India's Dedicated<br /><span className="text-brand-yellow">Drone, GIS &amp; AI</span><br />Industry Platform
            </h1>
            <div className="text-[11px] font-bold tracking-[0.14em] uppercase text-status-error mb-5">
            ...
            <div className="flex flex-wrap gap-6 sm:gap-8">
              {[['3','Industries Covered'],['50+','Interviews Published'],['100%','Industry Audience'],['14','Add-On Services']].map(([val, label]) => (
                <div key={label}>
                  <strong className="block text-2xl sm:text-3xl font-extrabold text-brand-yellow leading-none mb-1">{val}</strong>
                  <span className="text-[10px] text-white/50 uppercase tracking-wider font-medium">{label}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-white/[0.04] border border-brand-yellow/25 rounded-lg p-5">
            <div className="text-[10px] font-bold tracking-[0.12em] uppercase text-brand-yellow mb-4 pb-3 border-b border-brand-yellow/20">Quick Reference</div>
            {[
        ...
        <FeatGrid>
          {[
            ...
            { icon:'🔍', title:'Lead Generation', desc:'Buyer enquiries from Drone, GIS, and AI searches routed to subscribing companies. Full buyer data for Brand tier.',
              bullets:['Email notification on new enquiry','Full lead details on platform login','Monthly analytics for Expand and Brand','Full buyer contact data for Brand tier'] },
      ...
      {/* ── 02 DRONE ── */}
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-ink-offwhite -mx-0">
        <div className="max-w-7xl mx-auto">
          ...
          <FeatGrid>
            {[
              { badge:<Chip label="Manufacturers" color="bg-ink text-brand-yellow" />, title:'Drone Manufacturer Profiles', desc:'Dedicated profile pages for OEMs to list fixed-wing, multi-rotor, VTOL, and hybrid UAVs with full specifications and buyer enquiry forms.', bullets:['Product listing with full specifications','B2B buyer enquiry routing','Priority search ranking for Brand tier','Video interviews showcasing products'] },
              { badge:<Chip label="Service Providers" color="bg-ink text-brand-yellow" />, title:'Drone Service Companies', desc:'Agriculture spraying, inspection, survey, logistics, and media service companies list their capabilities and get enquiries from active buyers.', bullets:['Service listing by application sector','Lead notifications from active buyers','Category-wise discovery for buyers','Case study and editorial content'] },
              { badge:<Chip label="Training" color="bg-ink text-brand-yellow" />, title:'DGCA Training Institutes', desc:'DGCA-approved pilot training institutes, simulator labs, and drone academies reach students and career-changers through the platform.', bullets:['Institute profile and course listings','Career and training awareness content','Student enquiry routing','Social media promotion of programs'] },
              { badge:<Chip label="Components" color="bg-ink text-brand-yellow" />, title:'Drone Parts & Technology', desc:'Propulsion systems, frames, batteries, payloads, ground control systems, and ancillary tech suppliers reaching drone manufacturers.', bullets:['Component-level product listings','B2B supplier discovery','Product demo video content','Buyer enquiry management'] },
              { badge:<Chip label="Events" color="bg-ink text-brand-yellow" />, title:'Drone Expo Coverage', desc:'Official media partner for Drone Expo 2025 Mumbai and Drone Expo 2026 Bengaluru. 50+ interviews already produced.', bullets:['50+ interviews from Drone Expo 2025','Drone Expo 2026 Bengaluru: December 2026','Brand: expo stall branding','All tiers: digital event coverage'] },
              { badge:<Chip label="Media" color="bg-ink text-brand-yellow" />, title:'Drone Industry Media', desc:'Video interviews, short reels, editorial articles, press releases, and magazine ads — all produced by DroneTv for drone companies.', bullets:['YouTube interviews on @indiadronetv','Social posts across LinkedIn, Instagram, Facebook','Editorial articles on DroneTv.in','Quarterly magazine advertisements'] },
            ].map(c => <FeatCard key={c.title} {...c} />)}
          </FeatGrid>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <FeatGrid>
            {[
              ...
              { icon:'📈', title:'Lead Generation', desc:'Verified buyer enquiries from active searches across all three sectors routed directly to subscribing companies.', bullets:['B2B enquiry form on every profile','Email notification on new lead','Monthly reports for Expand and Brand','Full buyer data for Brand tier'] },
      ...
      {/* ── 06 MARKETPLACE ── */}
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-ink-offwhite">
        <div className="max-w-7xl mx-auto">
          ...
          <FeatGrid>
            {[
              ...
              { title:'Product & Service Listings', desc:'Drone hardware, GIS services, AI platforms, and supporting technologies listed and searchable by category across all sectors.', bullets:['Up to 10 listings (Reach)','Up to 25 listings (Expand)','Unlimited listings (Brand)','Searchable across all three sectors'] },
              ...
              { title:'Search & Category Placement', desc:'Featured placement (Expand) and priority search ranking with quarterly homepage feature (Brand) — applies across all three sectors.', bullets:['Featured category placement 3 months (Expand)','Quarterly homepage feature (Brand)','Priority search ranking within sector (Brand)','Industry vertical promotion (Brand)'] },
              { title:'Analytics & Lead Reports', desc:'Monthly lead count, profile views, and listing performance data for Expand and Brand subscribers across Drone, GIS, and AI sectors.', bullets:['Monthly lead summary & views (Expand)','Full analytics dashboard (Brand)','Full buyer name, email, phone (Brand)','Listing performance data (Brand)'] },
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-ink">
        <div className="max-w-7xl mx-auto">
          <SectionHead num="07" eyebrow="Media & Video" numColor="text-brand-yellow" eyebrowColor="text-white/40"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-ink-charcoal border-2 border-ink-charcoal rounded overflow-hidden">
            {[
              { tag:'Video', tagColor:'bg-status-error/60 text-status-error/40', title:'Video Interview (5-min)', desc:"CEO, technical lead, or product interviews for Drone, GIS, and AI companies. Published on YouTube and embedded on the company's DroneTv profile.", detail:'Expand: 1 | Brand: 2 | Promoted via social on publish week' },
              { tag:'Video', tagColor:'bg-status-error/60 text-status-error/40', title:'Short Reel (1-min)', desc:'Product reels, service showcases, or technology demos for all three sectors. Published on Instagram, Facebook, and cross-posted on LinkedIn.', detail:'Expand: 2 reels | Brand: 4 reels | All handles simultaneously' },
              { tag:'Social', tagColor:'bg-status-info/60 text-status-info/40', title:'Promotional Posts', desc:'Company posts published across LinkedIn, Instagram, and Facebook for Drone, GIS, and AI companies. Company tagged, website link included.', detail:'Reach: 2 (one-time) | Expand: 6/yr | Brand: 12/yr' },
              { tag:'Article', tagColor:'bg-status-success/60 text-status-success/40', title:'Editorial Articles', desc:'600–1000 word articles about the company — Drone technology, GIS methodology, AI solutions — published on DroneTv.in and promoted via social.', detail:'Expand: 1 article | Brand: 3 articles | Promoted on publish day' },
              { tag:'News', tagColor:'bg-status-success/60 text-status-success/40', title:'DroneTv News Posts', desc:'Short platform news posts for company announcements, product launches, certifications, and milestones — all three sectors.', detail:'Brand: up to 6/year | Company-specific items' },
              { tag:'PR', tagColor:'bg-status-success/60 text-status-success/40', title:'Press Releases', desc:'Press releases written and published on DroneTv.in for Drone, GIS, and AI company announcements and major news.', detail:'Brand: up to 6/yr | Add-on: ₹3,000 each' },
              { tag:'Social', tagColor:'bg-status-info/60 text-status-info/40', title:'Event Coverage Posts', desc:'Social posts covering product launches, awards, and exhibitions for Brand subscribers across Drone, GIS, and AI sectors.', detail:'Brand package only | Launches, awards, exhibitions' },
              { tag:'Magazine', tagColor:'bg-brand-gold/60 text-brand-yellow-soft', title:'Magazine Placements', desc:"Magazine advertisements, editorial articles, and cover page features in DroneTv's quarterly magazine — open to Drone, GIS, and AI companies equally.", detail:'Expand: Half-page x2 | Brand: Full-page x4 + editorial' },
            ].map(({ tag, tagColor, title, desc, detail }) => (
              <div key={title} className="bg-ink-charcoal p-5">
                <span className={`inline-block text-[9px] font-bold tracking-widest uppercase px-2 py-1 rounded mb-3 ${tagColor}`}>{tag}</span>
                <h4 className="text-[13px] font-bold text-brand-yellow mb-2 leading-snug">{title}</h4>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <FeatGrid>
            {[
              { title:'Industry Directory', desc:'Every subscriber appears in the magazine directory categorised as Drone, GIS, or AI — with logo, name, and tagline.', bullets:['Reach: 1 issue — logo, name, category','Expand: all issues — logo and tagline','Brand: all 4 issues — logo and tagline','Drone, GIS, and AI companies represented'] },
              { title:'Magazine Advertisements', desc:'Half-page (Expand) and full-page (Brand) advertisements open to Drone, GIS, and AI companies to reach the full cross-sector readership.', bullets:['Expand: half-page ad in 2 issues','Brand: full-page ad in all 4 issues','Print and digital distribution','Cross-sector Drone, GIS, AI readership'] },
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <FeatGrid>
            {[
              ...
              { title:'Coverage by Package Tier', desc:'What each package receives at DroneTv-partnered events. Applies to Drone, GIS, and AI exhibiting companies equally.', bullets:['Reach: digital event media coverage','Expand: digital event media coverage','Brand: stall branding + partner status + media credit','All tiers: content published across all platforms'] },
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-ink-light border-2 border-ink-light rounded overflow-hidden mb-8">
            {[
              ...
              <div key={num} className="bg-surface-card p-5 text-center">
                <div className="w-9 h-9 rounded-full bg-ink text-brand-yellow flex items-center justify-center text-sm font-extrabold mx-auto mb-3">{num}</div>
                <h4 className="text-[12px] font-bold text-ink mb-2">{title}</h4>
                <p className="text-[11px] text-ink-caption leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-ink-light border-2 border-ink-light rounded overflow-hidden">
            {[
              { badge:<Chip label="Reach" color="bg-ink text-brand-yellow" />, title:'Notification Only', bullets:['Email notification on new enquiry','Login to platform to view details','Drone, GIS, and AI companies covered'] },
              { badge:<Chip label="Expand" color="bg-ink text-brand-yellow" />, title:'Notification + Monthly Report', bullets:['Email notification on new enquiry','Login to view full details','Monthly enquiry count and profile views','All three sectors covered'] },
              { badge:<Chip label="Brand" color="bg-status-error text-white" />, title:'Full Buyer Data + Analytics', bullets:['Email notification on new enquiry','Full buyer name, company, email, phone on platform','Monthly analytics across all sectors','Listing performance data'] },
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-ink-offwhite">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-ink-light border-2 border-ink-light rounded overflow-hidden mb-8">
            {/* Reach */}
            <div className="bg-surface-card p-6">
              <span className="inline-block text-[9px] font-bold tracking-widest uppercase bg-brand-yellow text-ink px-2 py-1 rounded mb-3">Starter</span>
              <div className="text-xl font-extrabold text-ink mb-1">Reach</div>
              <div className="text-2xl font-extrabold text-ink mb-0.5">₹25,000</div>
              <div className="text-[10px] text-ink-caption mb-3">per year + GST | Drone · GIS · AI</div>
              <p className="text-[11.5px] text-ink-caption mb-4 pb-3 border-b border-ink-light">For Drone, GIS, or AI companies establishing their first digital presence on the platform.</p>
              <ul className="space-y-2">
                {['Verified company profile (Drone / GIS / AI)','Single-page website (basic)','Up to 10 product / service listings','B2B enquiry form','Lead notification email','2 social media posts (one-time)','Magazine directory listing (1 issue)'].map(item => (
                  <li key={item} className="flex items-start gap-2 text-[11.5px] text-ink-paragraph border-b border-ink-offwhite pb-2">
                ...
                {['Featured placement','Video interview or reel','Magazine advertisement','Monthly report'].map(item => (
                  <li key={item} className="flex items-start gap-2 text-[11.5px] text-ink-light border-b border-ink-offwhite pb-2">
                    <span className="font-bold flex-shrink-0">✗</span>{item}
                  </li>
                ))}
              </ul>
              <div className="mt-4 bg-ink-offwhite rounded p-3 text-[11px] font-semibold text-ink-paragraph">Market value ~₹54,000 | Save ₹29,000+</div>
            </div>

            {/* Expand — featured */}
            <div className="bg-ink text-white p-6">
              <span className="inline-block text-[9px] font-bold tracking-widest uppercase bg-status-error text-white px-2 py-1 rounded mb-3">Best Value</span>
              <div className="text-xl font-extrabold text-brand-yellow mb-1">Expand</div>
              ...
              <ul className="space-y-2">
                {['Featured profile + Featured Supplier badge','Enhanced website (gallery, services, lead form)','Up to 25 listings','Featured category placement — 3 months','Monthly lead summary report','6 social media posts per year','2 short reels (1-min each)','1 video interview (YouTube + embedded)','1 editorial article on DroneTv.in','Half-page magazine ad x 2 issues'].map(item => (
                  <li key={item} className="flex items-start gap-2 text-[11.5px] text-white/80 border-b border-white/7 pb-2">
                    <span className="text-brand-yellow font-bold flex-shrink-0">✓</span>{item}
                  </li>
                ))}
              ...
              <div className="mt-4 bg-brand-yellow/10 rounded p-3 text-[11px] font-semibold text-brand-yellow">Market value ~₹1,55,000 | Save ₹80,000+</div>
            </div>

            {/* Brand */}
            <div className="bg-surface-card p-6">
              <span className="inline-block text-[9px] font-bold tracking-widest uppercase bg-status-error text-white px-2 py-1 rounded mb-3">Premium</span>
              <div className="text-xl font-extrabold text-ink mb-1">Brand</div>
              <div className="text-2xl font-extrabold text-ink mb-0.5">₹1,50,000</div>
              <div className="text-[10px] text-ink-caption mb-3">per year + GST | Drone · GIS · AI</div>
              <p className="text-[11.5px] text-ink-caption mb-4 pb-3 border-b border-ink-light">Complete industry authority for category leaders in Drone, GIS, or AI — marketplace, media, magazine, expo, and full buyer data.</p>
              <ul className="space-y-2">
              ...
              <div className="mt-4 bg-ink-offwhite rounded p-3 text-[11px] font-semibold text-ink-paragraph">Market value ~₹4,35,000 | Save ₹2,85,000+</div>
            </div>
          </div>

          {/* Comparison Table */}
          <div className="mb-2">
            <div className="text-[10px] font-bold tracking-[0.16em] uppercase text-ink-caption mb-1">Full Matrix</div>
            <h3 className="text-lg font-bold text-ink mb-4">Package Feature Comparison — All Sectors</h3>
          </div>
          <div className="overflow-x-auto rounded border border-ink-light">
            <table className="w-full text-xs border-collapse min-w-[500px]">
              <thead>
                <tr>
                  <th className="bg-ink text-brand-yellow font-bold py-3 px-3 text-left text-[10.5px] tracking-wider min-w-[180px]">Feature</th>
                  {['Reach','Expand','Brand'].map(h => <th key={h} className="bg-ink text-brand-yellow font-bold py-3 px-3 text-center text-[10.5px] tracking-wider">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <React.Fragment key={cat}>
                    <tr><td colSpan={4} className="bg-ink-light text-ink-caption font-bold text-[10px] tracking-widest uppercase py-2 px-3">{cat}</td></tr>
                    {rows.map(([label, r, s, b], ri) => (
                      <tr key={ri} className={ri % 2 === 0 ? 'bg-surface-card' : 'bg-ink-offwhite'}>
                        <td className="py-2 px-3 text-ink-paragraph font-medium border-b border-ink-light text-left">{label}</td>
                        {[r, s, b].map((val, ci) => (
                          <td key={ci} className="py-2 px-3 text-center border-b border-ink-light">
                            {val === true ? <span className="text-status-success font-bold">Yes</span>
                              : val === false ? <span className="text-ink-light font-bold">—</span>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-ink-light border-2 border-ink-light rounded overflow-hidden">
            {[
              ...
              <div key={name} className="bg-surface-card px-4 py-3.5 flex items-center justify-between gap-2">
                <span className="text-[12px] font-semibold text-ink-paragraph">{name}</span>
                <span className="text-[11.5px] font-bold text-status-error whitespace-nowrap">{price}</span>
              </div>
            ))}
          </div>
          <div className="mt-5 bg-ink-offwhite border-l-4 border-brand-yellow px-4 py-3 text-xs text-ink-caption leading-relaxed rounded-r">
            Custom packages for <strong className="text-ink">OEMs, GIS organisations, AI enterprises, and government entities</strong> are available on request. Contact bd@dronetv.in or call +91 7520123555.
          </div>
        </div>
      </section>
      <GoldRule />
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-ink-offwhite">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { hBg:'bg-ink', hText:'text-brand-yellow', label:'Drone Industry Verticals', items:[
              ...
              <div key={label}>
                ...
                <div className="flex flex-col gap-px">
                  {items.map(([title, desc]) => (
                    <div key={title} className="bg-surface-card border border-ink-light px-4 py-3">
                      <h4 className="text-[12px] font-bold text-ink mb-0.5">{title}</h4>
                      <p className="text-[11px] text-ink-caption leading-snug">{desc}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-ink-light border-2 border-ink-light rounded overflow-hidden">
            {[
              { badge:'Drone', badgeColor:'bg-ink text-brand-yellow', title:'Drone Manufacturers', desc:'OEMs showcasing hardware and systems to qualified buyers across India.' },
              ...
              { badge:'All Sectors', badgeColor:'bg-ink-light text-ink-caption', title:'Startups & Innovators', desc:'Getting cross-sector exposure without high advertising costs.' },
            ].map(({ badge, badgeColor, title, desc }) => (
              <div key={title} className="bg-surface-card p-4">
                <span className={`inline-block text-[9px] font-bold px-2.5 py-1 rounded-full mb-2 tracking-wider ${badgeColor}`}>{badge}</span>
                <h4 className="text-[12.5px] font-bold text-ink mb-1">{title}</h4>
                <p className="text-[11px] text-ink-caption leading-snug">{desc}</p>
              </div>
            ))}
          </div>
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-ink-offwhite">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-px bg-ink-light border-2 border-ink-light rounded overflow-hidden">
            {[
              ...
              <div key={platform} className="bg-surface-card p-4 sm:p-5 text-center flex-1 min-w-[150px]">
                <div className="text-[10px] text-ink-caption uppercase tracking-widest mb-1">{platform}</div>
                <div className="text-[12.5px] font-bold text-ink mb-1.5">{handle}</div>
                <div className="text-[10.5px] text-ink-caption leading-snug">{note}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── CTA ── */}
      <section className="bg-brand-yellow py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 items-center">
          <div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-ink leading-tight tracking-tight mb-2">
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-3">
            <a href="mailto:bd@dronetv.in" className="bg-ink text-brand-yellow px-5 py-3 rounded font-semibold text-sm text-center hover:bg-ink-charcoal transition whitespace-nowrap">bd@dronetv.in</a>
            <a href="tel:+917520123555" className="bg-ink text-brand-yellow px-5 py-3 rounded font-semibold text-sm text-center hover:bg-ink-charcoal transition whitespace-nowrap">+91 7520123555</a>
            <button onClick={() => navigate('/form')} className="border-2 border-ink text-ink px-5 py-3 rounded font-semibold text-sm text-center hover:bg-ink-charcoal/10 transition whitespace-nowrap">Join DroneTv.in →</button>
          </div>
        </div>
      </section>

      {/* ── Footer strip ── */}
      <div className="bg-ink text-white/45 py-5 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-[11px]">
          <div><strong className="text-brand-yellow">Drone Academy Private Limited</strong> | 5A/6B White Waters, Timberlake Colony, Shaikpet, Hyderabad — 500008</div>
          <div className="whitespace-nowrap">www.dronetv.in | bd@dronetv.in | +91 7520123555 &nbsp; <strong className="text-brand-yellow">© 2024 Drone TV</strong></div>
        </div>
      </div>
