...
import { Search, ChevronDown, Filter, ChevronLeft, ChevronRight, Grid3x3, List, Bookmark, Eye, FileText, Tag, X } from 'lucide-react';
      ...
      <section style={PAGE_BG} className="flex flex-wrap items-center gap-2 px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1">
          ...
          <div className="hidden shrink-0 items-center gap-2 lg:flex">
            <ToolbarFilterDropdown label="Report Category" options={categories} selected={category === 'All' ? [] : [category]} onToggle={v => setCategory(v)} buttonClassName={`${BTN} flex items-center gap-4 text-xs`} />
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          <div className="flex items-center gap-3">
            ...
            <ShareMenu url={`${window.location.origin}/media/industry-reports/${item.contentId}`} title={item.title} buttonClassName="flex shrink-0 items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-700" iconClassName="size-3.5" />
          </div>
