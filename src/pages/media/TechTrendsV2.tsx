...
import { Search, ChevronDown, Filter, ChevronLeft, ChevronRight, Grid3x3, List, Heart, Eye, Share2, Copy, TrendingUp, Tag } from 'lucide-react';
import { fetchContent, MediaItem } from '../../lib/mediaApi';
import ToolbarFilterDropdown from '../../components/common/ToolbarFilterDropdown';

// Preview build at /media/tech-trends-v2 - same treatment as the other
// Media Hub V2 pages: real data from the tech-trends CMS, nothing
// fabricated. No second headline overlay (only one real title field per
// trend). Views real, comments dropped - no comment system exists.
...
const CAT_COLORS: Record<string, string> = {
  'drone technology': '#dc2626', 'gis & geospatial': '#15803d', 'ai in drones': '#0369a1',
  'robotics & automation': '#7c3aed', 'sensors & payloads': '#c2410c', 'communication & connectivity': '#be123c',
  'sustainability & green tech': '#15803d', 'future outlook': '#0369a1', 'digital health': '#0e7490',
};
...
const TechTrendsV2: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [allTrends, setAllTrends] = useState<MediaItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchContent('tech-trends', controller.signal).then(setAllTrends).catch(() => setAllTrends([])).finally(() => setLoading(false));
    return () => controller.abort();
  }, []);

  const categories = useMemo(() => Array.from(new Set(allTrends.map(t => t.category).filter(Boolean))) as string[], [allTrends]);
  ...
  }, [allTrends, category, search, sortBy]);
  ...
  if (loading) return (
    <div style={PAGE_BG} className="flex min-h-screen items-center justify-center">
      <p className="text-sm font-semibold text-slate-700">Loading Tech Trends...</p>
    </div>
  );

  const stats: [string, number, any][] = [
    ['Tech Trends', allTrends.length, TrendingUp],
    ...
    <div style={PAGE_BG} className="min-h-screen">
      ...
      <section aria-label="Tech trend statistics" className="flex min-h-[77px] flex-wrap items-center gap-3 bg-[#07130f] px-3 py-2 text-white sm:px-6">
        ...
        <div className="ml-auto min-w-[200px] shrink-0 text-right">
          <strong className="block text-sm leading-tight text-yellow-300">Drone Tech Trends</strong>
          <span className="block text-[11px] text-sky-300">Emerging Tech · Innovation</span>
        </div>
      </section>
      ...
      <section style={PAGE_BG} className="flex flex-wrap items-center gap-2 px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1">
          ...
          <div className="hidden shrink-0 items-center gap-2 lg:flex">
            <ToolbarFilterDropdown label="Trend Category" options={categories} selected={category === 'All' ? [] : [category]} onToggle={v => setCategory(v)} buttonClassName={`${BTN} flex items-center gap-4 text-xs`} />
          </div>
        </div>
        <label className="flex h-10 w-full items-center overflow-hidden rounded-lg border border-slate-200 bg-white md:w-[min(100%,360px)]">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search tech trends, innovations, emerging technologies..." className="min-w-0 flex-1 px-3 text-sm outline-none" />
      ...
      <main className="mx-auto grid max-w-[2100px] grid-cols-1 items-start gap-3 px-3 py-4 sm:px-6 lg:grid-cols-[255px_minmax(0,1fr)]">
        <aside className={`${sidebarOpen ? 'fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4 lg:static lg:z-auto lg:bg-transparent lg:p-0' : 'hidden'} self-start lg:block`}>
          <div className={sidebarOpen ? 'mx-auto max-w-sm rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm lg:mx-0 lg:max-w-none' : 'rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm'}>
            ...
            <section className="mb-4 border-b border-slate-200 pb-3">
              <h3 className="mb-3 text-xs font-extrabold">TREND CATEGORY</h3>
        ...
        <section className="min-w-0">
          <div className="mb-3 flex items-center justify-between gap-2">
            <h1 className="text-base font-extrabold">{filtered.length} Tech {filtered.length === 1 ? 'Trend' : 'Trends'}</h1>
          ...
          {visible.length === 0 ? (
            <p className="rounded-lg bg-white p-8 text-center">No tech trends found.</p>
          ) : (
            <div className={`grid gap-3 ${view === 'list' ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4'}`}>
              {visible.map((item, i) => (
                <TrendCardV2 key={`${item.contentId}-${i}`} item={item} onView={() => navigate(`/media/tech-trends/${item.contentId}`, { state: { item } })} />
              ))}
            </div>
          )}

          <div className="mt-5 grid min-w-0 grid-cols-1 items-center gap-4 rounded-lg px-3 py-5 sm:px-5 lg:grid-cols-[1fr_auto_1fr]">
            <strong className="text-sm">Showing {filtered.length ? first + 1 : 0}–{Math.min(first + perPage, filtered.length)} of {filtered.length.toLocaleString('en-IN')} trends</strong>
            <nav aria-label="tech trends pagination" className="flex min-w-0 flex-wrap items-center justify-center gap-1.5">
...
const TrendCardV2: React.FC<{ item: MediaItem; onView: () => void }> = ({ item, onView }) => {
  ...
  const handleShare = (e: React.MouseEvent) => {
    e.stopPropagation();
    const url = `${window.location.origin}/media/tech-trends/${item.contentId}`;
    ...
    <article onClick={onView} className="flex min-w-0 cursor-pointer flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative h-44 shrink-0 overflow-hidden bg-slate-100">
        {showImg ? (
          ...
          <div className="flex h-full w-full items-center justify-center"><TrendingUp className="size-10 text-slate-300" /></div>
        )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          ...
          <Link to={`/media/tech-trends/${item.contentId}`} state={{ item }} onClick={e => e.stopPropagation()} className="flex shrink-0 items-center rounded-lg bg-yellow-400 px-3 py-1.5 text-xs font-bold text-slate-900 hover:bg-yellow-300">
            Read More →
          </Link>
        </div>
      </div>
