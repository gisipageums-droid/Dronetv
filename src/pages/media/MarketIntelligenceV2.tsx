...
import { Search, ChevronDown, Filter, ChevronLeft, ChevronRight, Grid3x3, List, Heart, Eye, BarChart3, Tag, X } from 'lucide-react';
import { fetchContent, MediaItem } from '../../lib/mediaApi';
import ToolbarFilterDropdown from '../../components/common/ToolbarFilterDropdown';
import ShareMenu from '../../components/common/ShareMenu';

// Preview build at /media/market-intelligence-v2 - same treatment as
// News/Magazine/Video/Impact Stories V2: real data from the
// market-intelligence CMS, nothing fabricated. The reference's bold
// headline overlaid on the photo (separate from the real title below it)
// has no second real field to back it - only one real `title` per report,
// used once as the real heading, not duplicated as a fake tagline too.
// Views is real (same counter as every other media page). Comments stay
// dropped - no comment system exists anywhere in this backend.
...
const CAT_COLORS: Record<string, string> = {
  'market report': '#0369a1', 'industry trends': '#15803d', 'investment insights': '#7c3aed',
  'policy & regulation': '#dc2626', 'technology analysis': '#c2410c', 'regional analysis': '#0e7490',
  'sector focus': '#15803d', 'competitive landscape': '#be123c', 'corporate strategy': '#0369a1', geospatial: '#0e7490',
};
...
const MarketIntelligenceV2: React.FC = () => {
  ...
  const [allReports, setAllReports] = useState<MediaItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchContent('market-intelligence', controller.signal).then(setAllReports).catch(() => setAllReports([])).finally(() => setLoading(false));
    return () => controller.abort();
  }, []);

  const categories = useMemo(() => Array.from(new Set(allReports.map(r => r.category).filter(Boolean))) as string[], [allReports]);
  ...
  if (loading) return (
    <div style={PAGE_BG} className="flex min-h-screen items-center justify-center">
      <p className="text-sm font-semibold text-slate-700">Loading Market Intelligence...</p>
    </div>
  );

  const stats: [string, number, any][] = [
    ['Market Reports', allReports.length, BarChart3],
  ...
  return (
    <div style={PAGE_BG} className="min-h-screen">
      ...
      <section aria-label="Market intelligence statistics" className="flex min-h-[77px] flex-wrap items-center gap-3 bg-[#07130f] px-3 py-2 text-white sm:px-6">
        ...
        <div className="ml-auto min-w-[200px] shrink-0 text-right">
          <strong className="block text-sm leading-tight text-yellow-300">Drone Market Intelligence</strong>
          <span className="block text-[11px] text-sky-300">Reports · Trends · Analysis</span>
        </div>
      </section>
      ...
      <section style={PAGE_BG} className="flex flex-wrap items-center gap-2 px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1">
          ...
          <div className="hidden shrink-0 items-center gap-2 lg:flex">
            <ToolbarFilterDropdown label="Report Type" options={categories} selected={category === 'All' ? [] : [category]} onToggle={v => setCategory(v)} buttonClassName={`${BTN} flex items-center gap-4 text-xs`} />
          </div>
        </div>
        <label className="flex h-10 w-full items-center overflow-hidden rounded-lg border border-slate-200 bg-white md:w-[min(100%,360px)]">
          <span className="sr-only">Search market reports</span>
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search market reports, insights, trends..." className="min-w-0 flex-1 px-3 text-sm outline-none" />
      ...
      <main className="mx-auto grid max-w-[2100px] grid-cols-1 items-start gap-3 px-3 py-4 sm:px-6 lg:grid-cols-[255px_minmax(0,1fr)]">
        <aside onClick={e => { if (e.target === e.currentTarget) setSidebarOpen(false); }} className={`${sidebarOpen ? 'fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4 lg:static lg:z-auto lg:bg-transparent lg:p-0' : 'hidden'} self-start lg:block`}>
          <div className={sidebarOpen ? 'mx-auto max-w-sm rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm lg:mx-0 lg:max-w-none' : 'rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm'}>
            ...
            <section className="mb-4 border-b border-slate-200 pb-3">
              <h3 className="mb-3 text-xs font-extrabold">REPORT TYPE</h3>
        ...
        <section className="min-w-0">
          <div className="mb-3 flex items-center justify-between gap-2">
            <h1 className="text-base font-extrabold">{filtered.length} Market Intelligence {filtered.length === 1 ? 'Report' : 'Reports'}</h1>
          ...
          {visible.length === 0 ? (
            <p className="rounded-lg bg-white p-8 text-center">No market reports found.</p>
          ) : (
            <div className={`grid gap-3 ${view === 'list' ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4'}`}>
              {visible.map((item, i) => (
                <ReportCardV2 key={`${item.contentId}-${i}`} item={item} onView={() => navigate(`/media/market-intelligence/${item.contentId}`, { state: { item } })} />
              ))}
            </div>
          )}
          ...
          <div className="mt-5 grid min-w-0 grid-cols-1 items-center gap-4 rounded-lg px-3 py-5 sm:px-5 lg:grid-cols-[1fr_auto_1fr]">
            ...
            <nav aria-label="market intelligence pagination" className="flex min-w-0 flex-wrap items-center justify-center gap-1.5">
...
// reportCard() - matches the reference's card anatomy (ribbon badge, heart,
// photo, title, description, tag pills, views+share, View Report). No
// fabricated second headline overlay - see file-top note.
    ...
    <article onClick={onView} className="flex min-w-0 cursor-pointer flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative h-44 shrink-0 overflow-hidden bg-slate-100">
        {showImg ? (
          ...
          <div className="flex h-full w-full items-center justify-center"><BarChart3 className="size-10 text-slate-300" /></div>
        )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          <div className="flex items-center gap-3">
            ...
            <ShareMenu url={`${window.location.origin}/media/market-intelligence/${item.contentId}`} title={item.title} buttonClassName="flex shrink-0 items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-700" iconClassName="size-3.5" />
          </div>
          <Link to={`/media/market-intelligence/${item.contentId}`} state={{ item }} onClick={e => e.stopPropagation()} className="flex shrink-0 items-center rounded-lg bg-yellow-400 px-3 py-1.5 text-xs font-bold text-slate-900 hover:bg-yellow-300">
            View Report →
          </Link>
        </div>
      </div>
