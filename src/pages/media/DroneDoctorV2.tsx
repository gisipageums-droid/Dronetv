...
import { Search, ChevronDown, Filter, ChevronLeft, ChevronRight, Grid3x3, List, Heart, Eye, Share2, Copy, Stethoscope, Tag } from 'lucide-react';
import { fetchContent, MediaItem } from '../../lib/mediaApi';
import ToolbarFilterDropdown from '../../components/common/ToolbarFilterDropdown';

// Preview build at /media/drone-doctor-v2 - a BRAND NEW page (no live
// equivalent existed before this - grepped the whole codebase, confirmed).
// contentType 'drone-doctor' added to the shared media CMS's VALID_TYPES
// set (backend + frontend ContentType union) - same generic content model
// every other Media Hub page already uses, no new infrastructure. Same
// treatment as the others: real data, no second headline overlay, real
// views counter, no comments (no comment system exists).
...
const CAT_COLORS: Record<string, string> = {
  'technical support': '#dc2626', 'software & firmware': '#0369a1', 'dgca compliance': '#15803d',
  'maintenance & repair': '#7c3aed', 'payload & sensors': '#c2410c', 'data processing': '#0e7490',
  'buying guidance': '#be123c', 'training & certification': '#15803d',
};
...
const DroneDoctorV2: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [allTopics, setAllTrends] = useState<MediaItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchContent('drone-doctor', controller.signal).then(setAllTrends).catch(() => setAllTrends([])).finally(() => setLoading(false));
    return () => controller.abort();
  }, []);

  const categories = useMemo(() => Array.from(new Set(allTopics.map(t => t.category).filter(Boolean))) as string[], [allTopics]);
  ...
  }, [allTopics, category, search, sortBy]);
  ...
  if (loading) return (
    <div style={PAGE_BG} className="flex min-h-screen items-center justify-center">
      <p className="text-sm font-semibold text-slate-700">Loading Drone Doctor...</p>
    </div>
  );

  const stats: [string, number, any][] = [
    ['Help Topics', allTopics.length, Stethoscope],
  ...
  return (
    <div style={PAGE_BG} className="min-h-screen">
      ...
      <section aria-label="Drone doctor statistics" className="flex min-h-[77px] flex-wrap items-center gap-3 bg-[#07130f] px-3 py-2 text-white sm:px-6">
        ...
        <div className="ml-auto min-w-[200px] shrink-0 text-right">
          <strong className="block text-sm leading-tight text-yellow-300">Drone Doctor Help Line</strong>
          <span className="block text-[11px] text-sky-300">Technical Support · Guidance</span>
        </div>
      </section>
      ...
      <section style={PAGE_BG} className="flex flex-wrap items-center gap-2 px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1">
          ...
          <div className="hidden shrink-0 items-center gap-2 lg:flex">
            <ToolbarFilterDropdown label="Help Category" options={categories} selected={category === 'All' ? [] : [category]} onToggle={v => setCategory(v)} buttonClassName={`${BTN} flex items-center gap-4 text-xs`} />
          </div>
        </div>
        <label className="flex h-10 w-full items-center overflow-hidden rounded-lg border border-slate-200 bg-white md:w-[min(100%,360px)]">
          <span className="sr-only">Search tech trends</span>
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search help topics, issues, solutions, drone doctor..." className="min-w-0 flex-1 px-3 text-sm outline-none" />
      ...
      <main className="mx-auto grid max-w-[2100px] grid-cols-1 items-start gap-3 px-3 py-4 sm:px-6 lg:grid-cols-[255px_minmax(0,1fr)]">
        <aside className={`${sidebarOpen ? 'fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4 lg:static lg:z-auto lg:bg-transparent lg:p-0' : 'hidden'} self-start lg:block`}>
          <div className={sidebarOpen ? 'mx-auto max-w-sm rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm lg:mx-0 lg:max-w-none' : 'rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm'}>
            ...
            <section className="mb-4 border-b border-slate-200 pb-3">
              <h3 className="mb-3 text-xs font-extrabold">HELP CATEGORY</h3>
        ...
        <section className="min-w-0">
          <div className="mb-3 flex items-center justify-between gap-2">
            <h1 className="text-base font-extrabold">{filtered.length} Help {filtered.length === 1 ? 'Topic' : 'Topics'}</h1>
          ...
          {visible.length === 0 ? (
            <p className="rounded-lg bg-white p-8 text-center">No help topics found.</p>
          ) : (
            <div className={`grid gap-3 ${view === 'list' ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4'}`}>
              {visible.map((item, i) => (
                <HelpCardV2 key={`${item.contentId}-${i}`} item={item} onView={() => navigate(`/media/drone-doctor/${item.contentId}`, { state: { item } })} />
              ))}
            </div>
          )}

          <div className="mt-5 grid min-w-0 grid-cols-1 items-center gap-4 rounded-lg px-3 py-5 sm:px-5 lg:grid-cols-[1fr_auto_1fr]">
            <strong className="text-sm">Showing {filtered.length ? first + 1 : 0}–{Math.min(first + perPage, filtered.length)} of {filtered.length.toLocaleString('en-IN')} help topics</strong>
            <nav aria-label="drone doctor pagination" className="flex min-w-0 flex-wrap items-center justify-center gap-1.5">
...
const HelpCardV2: React.FC<{ item: MediaItem; onView: () => void }> = ({ item, onView }) => {
  ...
  const handleShare = (e: React.MouseEvent) => {
    e.stopPropagation();
    const url = `${window.location.origin}/media/drone-doctor/${item.contentId}`;
    ...
    <article onClick={onView} className="flex min-w-0 cursor-pointer flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative h-44 shrink-0 overflow-hidden bg-slate-100">
        {showImg ? (
          ...
          <div className="flex h-full w-full items-center justify-center"><Stethoscope className="size-10 text-slate-300" /></div>
        )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={handleShare} aria-label="Copy link" className="flex shrink-0 items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-700">
              {copied ? <Copy className="size-3.5" /> : <Share2 className="size-3.5" />}
            </button>
          </div>
          <Link to={`/media/drone-doctor/${item.contentId}`} state={{ item }} onClick={e => e.stopPropagation()} className="flex shrink-0 items-center rounded-lg bg-yellow-400 px-3 py-1.5 text-xs font-bold text-slate-900 hover:bg-yellow-300">
            Get Help →
          </Link>
        </div>
      </div>
