...
import { Search, ChevronDown, Filter, ChevronLeft, ChevronRight, Grid3x3, List, Heart, Eye, Sparkles, Tag, MapPin, X } from 'lucide-react';
...
// fabricated. The reference's bold marketing tagline overlaid on the photo
// ("Higher Yields Brighter Futures", separate from the real article title
// below it) has no real second-headline field to back it - only one real
// `title` exists per story, so it's used once, as the real heading below
// the photo, not duplicated as a fake overlay too. Location badge on the
// photo IS real (MediaItem.location). Views is real (same counter as
// News/Magazine/Video). Comments stay dropped - no comment system exists.
...
const CAT_COLORS: Record<string, string> = {
  agriculture: '#15803d', 'disaster response': '#0369a1', 'environmental impact': '#16a34a',
  'healthcare impact': '#be123c', infrastructure: '#c2410c', 'skills & education': '#7c3aed',
  'economic impact': '#0e7490', 'defence & security': '#475569', 'survey and gis': '#0369a1',
  'training institute': '#7c3aed', manufacturing: '#b45309',
};
...
const ImpactStoriesV2: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [allStories, setAllStories] = useState<MediaItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchContent('impact-story', controller.signal).then(setAllStories).catch(() => setAllStories([])).finally(() => setLoading(false));
  ...
  if (loading) return (
    <div style={PAGE_BG} className="flex min-h-screen items-center justify-center">
      <p className="text-sm font-semibold text-slate-700">Loading Impact Stories...</p>
    </div>
  );

  const stats: [string, number, any][] = [
    ['Impact Stories', allStories.length, Sparkles],
    ['Categories', categories.length, Tag],
    ['Locations', new Set(allStories.map(s => s.location).filter(Boolean)).size, MapPin],
  ];
  ...
  return (
    <div style={PAGE_BG} className="min-h-screen">
      ...
      <section aria-label="Impact story statistics" className="flex min-h-[77px] flex-wrap items-center gap-3 bg-[#07130f] px-3 py-2 text-white sm:px-6">
        ...
        <a href="mailto:bd@dronetv.in?subject=Submit Impact Story" className="flex h-9 shrink-0 items-center justify-center rounded-lg bg-[#ffdf00] px-4 text-sm font-extrabold text-black">Share Your Story</a>
        <div className="ml-auto min-w-[200px] shrink-0 text-right">
          <strong className="block text-sm leading-tight text-yellow-300">Drone Impact Stories</strong>
          <span className="block text-[11px] text-sky-300">Real-World Impact · India Focused</span>
        </div>
      </section>
      ...
      <section style={PAGE_BG} className="flex flex-wrap items-center gap-2 px-3 py-3 sm:px-6">
        ...
        <label className="flex h-10 w-full items-center overflow-hidden rounded-lg border border-slate-200 bg-white md:w-[min(100%,360px)]">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search impact stories, companies, projects..." className="min-w-0 flex-1 px-3 text-sm outline-none" />
      ...
      <main className="mx-auto grid max-w-[2100px] grid-cols-1 items-start gap-3 px-3 py-4 sm:px-6 lg:grid-cols-[255px_minmax(0,1fr)]">
        <aside onClick={e => { if (e.target === e.currentTarget) setSidebarOpen(false); }} className={`${sidebarOpen ? 'fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4 lg:static lg:z-auto lg:bg-transparent lg:p-0' : 'hidden'} self-start lg:block`}>
          <div className={sidebarOpen ? 'mx-auto max-w-sm rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm lg:mx-0 lg:max-w-none' : 'rounded-xl border border-yellow-300 bg-[#fffef0] p-4 shadow-sm'}>
            ...
            <section className="mb-4 border-b border-slate-200 pb-3">
              <h3 className="mb-3 text-xs font-extrabold">IMPACT CATEGORY</h3>
        ...
        <section className="min-w-0">
          <div className="mb-3 flex items-center justify-between gap-2">
            <h1 className="text-base font-extrabold">{filtered.length} {filtered.length === 1 ? 'Impact Story' : 'Impact Stories'}</h1>
          ...
          {visible.length === 0 ? (
            <p className="rounded-lg bg-white p-8 text-center">No impact stories found.</p>
          ) : (
            <div className={`grid gap-3 ${view === 'list' ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4'}`}>
              {visible.map((item, i) => (
                <ImpactCardV2 key={`${item.contentId}-${i}`} item={item} onRead={() => navigate(`/media/impact-stories/${item.contentId}`, { state: { item } })} />
              ))}
            </div>
          )}

          <div className="mt-5 grid min-w-0 grid-cols-1 items-center gap-4 rounded-lg px-3 py-5 sm:px-5 lg:grid-cols-[1fr_auto_1fr]">
            <strong className="text-sm">Showing {filtered.length ? first + 1 : 0}–{Math.min(first + perPage, filtered.length)} of {filtered.length.toLocaleString('en-IN')} stories</strong>
            <nav aria-label="impact stories pagination" className="flex min-w-0 flex-wrap items-center justify-center gap-1.5">
          ...
          <div className="mt-8 flex flex-col items-start gap-4 rounded-xl bg-slate-900 p-6 md:flex-row md:items-center md:justify-between">
            <div>
              <h3 className="mb-1 text-base font-bold text-white">Share Your Impact Story</h3>
              <p className="max-w-lg text-sm text-white/60">Has your company achieved measurable results using drones — yield improvement, time savings, cost reduction, or business growth? DroneTv.in publishes verified impact stories from across India&rsquo;s drone ecosystem.</p>
            </div>
            <div className="flex shrink-0 gap-3">
              <a href="mailto:bd@dronetv.in?subject=Submit Impact Story" className="rounded-lg bg-yellow-400 px-4 py-2 text-sm font-bold text-slate-900 transition-colors hover:bg-yellow-300">Share Your Story</a>
              <Link to="/media/video-spotlight" className="rounded-lg border border-white/20 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-white/10">Video Interviews →</Link>
            </div>
          </div>
        </section>
      </main>
...
// impactCard() - matches the reference's card anatomy (ribbon badge, heart,
// photo with location badge, title, description, tag pills, views+share,
// Read Story). No fabricated second headline overlay - see file-top note.
const ImpactCardV2: React.FC<{ item: MediaItem; onRead: () => void }> = ({ item, onRead }) => {
    ...
    <article onClick={onRead} className="flex min-w-0 cursor-pointer flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative h-44 shrink-0 overflow-hidden bg-slate-100">
        {showImg ? (
          ...
          <div className="flex h-full w-full items-center justify-center"><Sparkles className="size-10 text-slate-300" /></div>
        )}
        ...
        {item.location && (
          <span className="absolute bottom-3 right-3 flex items-center gap-1 rounded bg-black/70 px-2 py-1 text-[11px] font-semibold text-white">
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          <div className="flex items-center gap-3">
            ...
            <ShareMenu url={`${window.location.origin}/media/impact-stories/${item.contentId}`} title={item.title} buttonClassName="flex shrink-0 items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-700" iconClassName="size-3.5" />
          </div>
          <Link to={`/media/impact-stories/${item.contentId}`} state={{ item }} onClick={e => e.stopPropagation()} className="flex shrink-0 items-center rounded-lg bg-yellow-400 px-3 py-1.5 text-xs font-bold text-slate-900 hover:bg-yellow-300">
            Read Story →
          </Link>
        </div>
      </div>
...
export default ImpactStoriesV2;
