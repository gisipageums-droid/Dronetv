...
// Per-service overrides — set only the ones you've cut over to the
// self-hosted VPS. Falls back to the global BACKEND switch, then to Lambda
// (null) when neither is set. .trim() guards against a stray trailing
// newline in a pasted Vercel env var value silently breaking every URL.
const SERVICE_OVERRIDES: Record<string, string> = {
  auth: (import.meta.env.VITE_BACKEND_URL_AUTH || '').trim(),
  company: (import.meta.env.VITE_BACKEND_URL_COMPANY || '').trim(),
  professional: (import.meta.env.VITE_BACKEND_URL_PROFESSIONAL || '').trim(),
  events: (import.meta.env.VITE_BACKEND_URL_EVENTS || '').trim(),
  media: (import.meta.env.VITE_BACKEND_URL_MEDIA || '').trim(),
  leads: (import.meta.env.VITE_BACKEND_URL_LEADS || '').trim(),
  payment: (import.meta.env.VITE_BACKEND_URL_PAYMENT || '').trim(),
  admin: (import.meta.env.VITE_BACKEND_URL_ADMIN || '').trim(),
  jobApplications: (import.meta.env.VITE_BACKEND_URL_JOB_APPLICATIONS || '').trim(),
};
