...
// Default data
const defaultData: SpeakersData = {
  subtitle: "Featured Speakers",
  heading: "Learn From the Best",
  description: "Hear from industry pioneers, innovators, and thought leaders who are shaping the future of business and technology.",
  speakers: [
    {
      id: '1',
...
export function SpeakersSection({ speakersData, onStateChange, userId, professionalId, templateSelection }: SpeakersProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [isUploading, setIsUploading] = useState(false);
  const [dataLoaded, setDataLoaded] = useState(false);
  const [data, setData] = useState<SpeakersData>(defaultData);
  const [tempData, setTempData] = useState<SpeakersData>(defaultData);
  ...
  const handleSave = async () => {
    try {
      ...
      if (pendingImages.length > 0) {
        ...
        tempData.speakers = updatedSpeakers;
      }
      ...
      setPendingImages([]);
      setIsEditing(false);
      toast.success('Speakers section saved successfully');
    } catch (error) {
  ...
  const applyCrop = async () => {
    try {
      ...
      toast.success('Speaker image updated! Click Save to confirm.');
            ...
            {displayData.speakers.map((speaker) => (
              ...
              >
                ...
                <div className={`relative h-40 bg-yellow-200 flex items-center justify-center ${speaker.color}`}>
                  ...
                  {/* Social Icons (View Mode) */}
                  {!isEditing && (
                    <div className="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      {speaker.socialLinks.linkedin && (
                        <a
                          href={speaker.socialLinks.linkedin}
                          className="w-8 h-8 bg-white/90 rounded-full flex items-center justify-center hover:bg-white transition-colors shadow-md"
                          target="_blank"
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      ...
                      <div className="pt-3 border-t border-gray-100">
                        <label className="block text-sm text-gray-600 mb-2">Card Color:</label>
                        <select
                          value={speaker.color}
                          onChange={(e) => updateSpeakerColor(speaker.id, e.target.value)}
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm text-gray-600">Social Links:</label>
                        <EditableText
                          value={speaker.socialLinks.linkedin || ''}
                          onChange={(value) => updateSocialLink(speaker.id, 'linkedin', value)}
                          className="text-sm"
                          placeholder="LinkedIn URL"
                        />
                        <EditableText
                          value={speaker.socialLinks.twitter || ''}
                          onChange={(value) => updateSocialLink(speaker.id, 'twitter', value)}
                          className="text-sm"
