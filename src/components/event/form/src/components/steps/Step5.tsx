...
export const Step5 = ({
  ...
  const baseTextareaClasses =
    "border border-amber-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-amber-400 transition text-sm w-full min-h-[80px] resize-vertical";
    ...
    return (
      <div className="space-y-6 p-6 bg-yellow-50 rounded-xl shadow-md">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Add a background video URL for your event (YouTube, Vimeo, or direct
            video link)
          </p>
        </div>
  ...
  const renderExhibitorInterviews = () => {
    return (
      <div className="space-y-6 p-6 bg-yellow-50 rounded-xl shadow-md">
        ...
        {exhibitorInterviews.map(
          (interview: ExhibitorInterview, index: number) => (
            <div
              key={index}
              className="p-4 bg-white rounded-lg border border-amber-200 space-y-4"
            >
              <div className="flex justify-between items-center">
                <h4 className="font-semibold text-slate-800">
                  Interview {index + 1}
        ...
        <button
          type="button"
          onClick={() =>
            addArrayItem("exhibitorInterviews", {
              videoTitle: "",
              videoUrl: "",
            })
          }
  ...
  const renderMediaGallery = () => {
    ...
    const detectFileType = (mediaItem: MediaGalleryItem): string => {
      ...
      if (mediaItem.file) {
        if (mediaItem.file.type.startsWith("image/")) {
          ...
          mediaItem.file.type.includes("word") ||
          mediaItem.file.name?.match(/\.(doc|docx)$/)
        ) {
          return "word";
        } else if (
          mediaItem.file.type.includes("excel") ||
          mediaItem.file.type.includes("spreadsheet") ||
          mediaItem.file.name?.match(/\.(xls|xlsx)$/)
        ) {
          return "excel";
        } else if (
          mediaItem.file.type.includes("powerpoint") ||
          mediaItem.file.type.includes("presentation") ||
          mediaItem.file.name?.match(/\.(ppt|pptx)$/)
        ) {
          return "powerpoint";
        } else if (
          mediaItem.file.type === "text/plain" ||
          mediaItem.file.name?.endsWith(".txt")
        ) {
          return "text";
        } else {
          return "document";
      ...
      if (mediaItem.mediaUrl) {
        ...
        // Check for PowerPoint
          ...
          lowerFileName.match(/\.(ppt|pptx)$/) ||
          mediaItem.mediaType === "powerpoint"
        ) {
          return "powerpoint";
        }
        // Check for text files
    ...
    const renderMediaSlot = (
      ...
      return (
        <div
          key={index}
          className="p-4 bg-white rounded-lg border border-amber-200 space-y-4 relative"
        >
          {/* Loading Overlay */}
          {media.uploading && (
            <div className="absolute inset-0 bg-white bg-opacity-75 rounded-lg flex items-center justify-center z-10">
            ...
            {!media.mediaUrl ? (
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 rounded-lg p-3">
                  ...
                  {process.env.NODE_ENV === "development" && (
                    <div className="text-xs bg-yellow-100 p-2 rounded mb-2">
                      <strong>Debug:</strong> Type: {actualFileType}, File:{" "}
                      {media.fileName}
                    </div>
                  )}
    ...
    return (
      <div className="space-y-8 p-6 bg-yellow-50 rounded-xl shadow-md">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Upload images and documents for your event (3 images and 3 documents
            maximum)
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {documentSlots.map(
              (media: MediaGalleryItem, index: number) =>
        ...
        <div className="p-4 bg-amber-100 rounded-lg border border-amber-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            {mediaGallery
              .slice(0, 3)
              .map((media: MediaGalleryItem, index: number) => (
                <div key={index} className="flex items-center gap-2">
                  <div
  ...
  const renderInternationalContacts = () => {
    return (
      <div className="space-y-6 p-6 bg-yellow-50 rounded-xl shadow-md">
        ...
        {internationalContacts.map(
          (contact: InternationalContact, index: number) => (
            <div
              key={index}
              className="p-4 bg-white rounded-lg border border-amber-200 space-y-4"
            >
              <div className="flex justify-between items-center">
                ...
                <button
                  type="button"
                  onClick={() =>
                    removeArrayItem("internationalContacts", index)
                  }
                  className="text-red-500 hover:text-red-700 text-sm font-medium transition flex items-center"
                >
                  <Trash2 className="w-4 h-4 mr-1" />
        ...
        <button
          type="button"
          onClick={() =>
            addArrayItem("internationalContacts", {
              name: "",
              phone: "",
              email: "",
              organization: "",
                ...
                <span className="font-medium">
                  {
                    mediaGallery
                      .slice(0, 3)
                      .filter((m: MediaGalleryItem) => m.uploaded).length
                  }
                  /3
                </span>
              </div>
