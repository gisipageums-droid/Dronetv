...
// const Step6: React.FC<Step6Props> = ({ className = '' }) => {
...
//     })),
//     showFullText, // since you said this holds what needs to be sent
//   };
...
import * as pdfjsLib from 'pdfjs-dist';
import workerUrl from 'pdfjs-dist/build/pdf.worker.min.mjs?url';
import { useForm } from '../../context/FormContext'; // adjust path if needed
...
const Step6: React.FC<Step6Props> = ({ className = '', onSubmit }) => {
  ...
  // ✅ Auto-send extracted data to parent whenever documents or showFullText change
  // useEffect(() => {
  //   if (onSubmit) {
  //     const payload = {
  //       documents: documents.map(doc => ({
  ...
  //         uploadDate: doc.uploadDate,
  //       })),
  //       showFullText, // ✅ send this value to parent as well
  //     };
  //     onSubmit(payload);
  //   }
  ...
  console.log('Updated FormContext resume:', documents); // 👈 assurance console.log
}, [documents, showFullText]);
  ...
  Check FormContext
                  ...
                  <div className="text-center py-12">
                    <FileText className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                    <h3 className="text-xl font-semibold text-gray-600 mb-2">No Document Selected</h3>
                    <p className="text-gray-500">Upload a document to view its extracted text.</p>
                  </div>
                )}
              </div>
            </div>
