...
export function Footer({ footerData, onStateChange }: FooterProps) {
  ...
  const scheduleAutoSave = useCallback((updatedData: FooterData) => {
  ...
  useEffect(() => {
    if (footerData && !dataLoaded) {
      console.log('Initializing Footer data from props:', footerData);
      setData(footerData);
      setTempData(footerData);
      lastSavedDataRef.current = footerData;
      setDataLoaded(true);
  ...
  }, [footerData, dataLoaded, onStateChange]);
  ...
  useEffect(() => {
    const loadDataIfNeeded = async () => {
      if (isVisible && !dataLoaded && !footerData && !isLoading) {
        setIsLoading(true);
        try {
          console.log('Loading default Footer data');
          setData(defaultFooterData);
          setTempData(defaultFooterData);
          lastSavedDataRef.current = defaultFooterData;
          setDataLoaded(true);
          
          if (onStateChange) {
            onStateChange(defaultFooterData);
          }
        } finally {
          setIsLoading(false);
        }
    ...
    loadDataIfNeeded();
  }, [isVisible, dataLoaded, footerData, isLoading, onStateChange, defaultFooterData]);
            ...
            {isEditing ? (
              <div className="space-y-2">
                ...
                <FooterEditableText
                  ...
                  onChange={(value) => updateContactInfo('location', value)}
                  charLimit={FOOTER_TEXT_LIMITS.CONTACT_FIELD}
                  className="text-sm text-gray-400"
                ...
                <FooterEditableText
                  ...
                  onChange={(value) => updateContactInfo('availability', value)}
                  charLimit={FOOTER_TEXT_LIMITS.CONTACT_FIELD}
                  className="text-sm text-gray-400"
              ...
              <div className="space-y-2 text-sm text-gray-400">
                {displayData.contactInfo.email && <p>{displayData.contactInfo.email}</p>}
                {displayData.contactInfo.location && <p>{displayData.contactInfo.location}</p>}
                {displayData.contactInfo.availability && <p>{displayData.contactInfo.availability}</p>}
              </div>
            )}
          </div>
                ...
                {displayData.socialLinks.map((link, index) => (
                  <div key={index} className="flex gap-2">
                    <FooterEditableText
                      ...
                      charLimit={FOOTER_TEXT_LIMITS.SOCIAL_LABEL}
                      className="flex-1 text-sm text-gray-400"
                ...
                <Button
                  onClick={() => {
                    setTempData(prev => {
                      const updated = {
                        ...prev,
                        socialLinks: [...prev.socialLinks, { icon: "default", label: "New Social", href: "#", color: "#ffffff" }]
                      };
                      scheduleAutoSave(updated);
                      return updated;
                    });
