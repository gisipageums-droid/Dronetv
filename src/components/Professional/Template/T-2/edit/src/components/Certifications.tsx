...
export function Certifications({ certData, onStateChange, userId, professionalId, templateSelection }: CertificationsProps) {
  ...
  const uploadPendingImages = async (): Promise<boolean> => {
    ...
    try {
      ...
      for (let i = 0; i < pendingEntries.length; i++) {
        const [certId, file] = pendingEntries[i];
  ...
  const autoUploadImage = async (file: File, certId: string): Promise<string | null> => {
    if (!userId || !professionalId || !templateSelection) {
  ...
  const applyCrop = async () => {
    try {
      ...
      const s3Url = await autoUploadImage(file, currentCroppingCert);
