...
export function Projects({ projectsData, onStateChange, userId, professionalId, templateSelection }: ProjectsProps) {
  ...
  const performAutoSave = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error auto-saving projects section:', error);
      if (isMountedRef.current) {
  ...
  const uploadImageToS3 = useCallback(async (file: File, projectId: string): Promise<string> => {
    if (!userId || !professionalId || !templateSelection) {
  ...
  const handleAutoImageUpload = useCallback(async (file: File, projectId: string) => {
    try {
      setIsUploading(true);
  ...
  // Apply crop and auto-upload image - UPDATED WITH ZOOM OUT LOGIC
  const applyCrop = async () => {
    try {
      if (!imageToCrop || !croppedAreaPixels || !currentCroppingProject) return;
      ...
      console.log('Project image cropped, starting auto-upload...', file);
      
      // Auto-upload the cropped image to S3
      await handleAutoImageUpload(file, currentCroppingProject);
  ...
  const handleSave = async () => {
    try {
      ...
      // Upload images for projects with pending files that weren't auto-uploaded
      for (const [projectId, file] of Object.entries(pendingImageFiles)) {
        if (!userId || !professionalId || !templateSelection) {
          toast.error('Missing user information. Please refresh and try again.');
          return;
        ...
        const s3Url = await uploadImageToS3(file, projectId);
        updatedData.projects = updatedData.projects.map(project =>
          project.id.toString() === projectId ? { ...project, image: s3Url } : project
        );
      }
