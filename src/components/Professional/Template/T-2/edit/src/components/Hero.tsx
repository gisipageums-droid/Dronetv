...
export function Hero({ heroData, onStateChange, userId, professionalId, templateSelection }: HeroProps) {
  ...
  const performAutoSave = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        ...
        // Transform data back to backend format before notifying parent
  ...
  }, [hasUnsavedChanges, isEditing, tempData, onStateChange, transformToBackendFormat]);
  ...
  const handleAutoImageUpload = useCallback(async (file: File) => {
    try {
      setIsUploading(true);
  ...
  const applyCrop = async () => {
    try {
      ...
      console.log('Hero image cropped, starting auto-upload...', file);
      
      // Auto-upload the cropped image to S3
      await handleAutoImageUpload(file);
  ...
  const handleSave = async () => {
    try {
      ...
      // Upload image only if there's a pending file that wasn't auto-uploaded
      let updatedData = { ...tempData };
      
      if (pendingImageFile && !isUploading) {
        setIsUploading(true);
        const s3Url = await uploadImageToS3(pendingImageFile);
        updatedData.image = s3Url;
        setPendingImageFile(null);
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full mt-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            {isEditing && (isAutoSaving || hasUnsavedChanges) && (
              <motion.div 
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex items-center gap-2 px-3 py-2 text-xs bg-yellow-100 text-yellow-800 rounded-md border border-yellow-300 w-fit"
              >
                {isAutoSaving ? (
                  <>
