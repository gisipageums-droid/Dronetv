...
const Hero: React.FC<HeroProps> = ({ content, onSave, userId }) => {
  ...
  const [originalContent, setOriginalContent] = useState<HeroContent>(content); // ✅ store last saved version
  ...
  const handleSave = () => {
    ...
    setOriginalContent(heroContent); // ✅ update the original state after saving
    toast.success("Hero section updated successfully!");
    setIsEditing(false);
  };

  const handleCancel = () => {
    setHeroContent(originalContent); // ✅ revert to last saved content
    setIsEditing(false);
            ...
            <div className="space-y-6">
              {isEditing ? (
                ...
                <h1 className="text-4xl lg:text-6xl font-bold text-gray-900 dark:text-white">
                  {/* Hi, I'm{" "} */}
                  <span className="text-blue-500 dark:text-orange-500">{heroContent.name}</span>
                </h1>
              )}
