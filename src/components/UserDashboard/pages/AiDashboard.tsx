...
const statusConfig: Record<TabStatus, { label: string; color: string; bg: string }> = {
  active:        { label: "Active", color: "text-status-success", bg: "bg-status-success/15"  },
  beta:          { label: "Beta",   color: "text-status-info",  bg: "bg-status-info/15"   },
  "coming-soon": { label: "Soon",   color: "text-brand-gold", bg: "bg-brand-yellow-soft"  },
};
...
function ComingSoonPanel({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full py-24 text-center">
      ...
      <h2 className="text-lg font-bold text-ink mb-1">{label} — Coming Soon</h2>
      <p className="text-sm text-ink-caption max-w-xs">This feature is under development and will be available soon.</p>
    </div>
  );
}
...
function PanelLoader() {
  return (
    <div className="flex items-center justify-center h-full py-24 text-ink-light">
...
const AiDashboard: React.FC = () => {
  ...
  return (
    <div className="flex h-full bg-surface-card rounded-lg overflow-hidden">
      {/* ── Sidebar ── */}
      <aside className="w-56 shrink-0 bg-surface-main border-r border-brand-yellow-soft overflow-y-auto flex flex-col">
        <div className="px-4 py-3 border-b border-brand-yellow-soft">
          <h2 className="text-xs font-bold text-brand-gold uppercase tracking-widest mb-2">AI Suite</h2>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-brand-yellow-soft rounded-lg flex items-center justify-center shrink-0">
              {role === "admin" ? <Crown size={14} className="text-brand-gold" /> : <User size={14} className="text-brand-gold" />}
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-ink-charcoal truncate">{displayName}</p>
              {role === "admin"
                ? <span className="text-[10px] font-bold text-brand-gold">Admin</span>
                : <span className="text-[10px] text-ink-caption">User</span>
              }
            </div>
          </div>
        </div>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-left transition-all duration-150 border-l-4 ${
                      isGroupActive
                        ...
                        : "border-transparent text-ink-paragraph hover:bg-brand-yellow-soft hover:border-brand-yellow-soft"
                    }`}
                  >
                    ...
                    <ChevronDown
                      size={14}
                      className={`shrink-0 text-ink-caption transition-transform duration-200 ${isGroupExpanded ? "rotate-180" : ""}`}
                    />
                  </button>

                  {/* Children */}
                  {isGroupExpanded && (
                    <div className="bg-surface-main/60 border-l-2 border-brand-yellow-soft ml-4">
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center gap-2 pl-4 pr-3 py-2 text-left transition-all duration-150 border-l-2 ${
                              isActive
                                ? "border-brand-gold bg-brand-yellow text-ink"
                                : "border-transparent text-ink-paragraph hover:bg-brand-yellow-soft hover:text-ink-charcoal"
                            }`}
                          >
