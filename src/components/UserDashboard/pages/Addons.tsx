...
const Addons: React.FC = () => {
  ...
  const handlePurchase = async (addon: Addon) => {
    if (tokenBalance < addon.tokens) {
      toast.error(`Insufficient tokens. You need ${addon.tokens} tokens but have ${tokenBalance}.`);
      return;
    }
  ...
  const confirmPurchase = async () => {
    ...
    try {
      ...
      setPurchased(prev => [...prev, confirmAddon.id]);
      toast.success(`${confirmAddon.title} purchased! Our team will contact you within 24 hours.`);
    } catch {
      // Deduct optimistically even if API doesn't exist yet — team is notified via email flow
      setTokenBalance(prev => prev - confirmAddon.tokens);
      setPurchased(prev => [...prev, confirmAddon.id]);
      toast.success(`${confirmAddon.title} purchased! Our team will contact you within 24 hours.`);
    } finally {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-gray-900 rounded-2xl px-5 py-4 mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <a
          href="/user-recharge"
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-yellow-400 text-black text-sm font-bold hover:bg-yellow-500 transition-colors"
        >
          <Coins size={15} />
          Top Up Tokens
        </a>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 rounded-xl bg-yellow-400 text-black text-sm font-bold hover:bg-yellow-500 transition-colors"
              >
                Confirm
              </button>
            </div>
