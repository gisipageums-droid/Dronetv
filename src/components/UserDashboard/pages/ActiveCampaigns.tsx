    ...
    <div className="min-h-screen bg-ink p-4 sm:p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black text-white flex items-center gap-2">
              <BarChart2 size={22} className="text-brand-yellow" /> Active Campaigns
            </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { label: "Active Campaigns", value: totalActive, icon: BarChart2, color: "text-brand-yellow" },
            { label: "Active Bids",      value: activeBids.length,       icon: Target, color: "text-status-info" },
            { label: "Active Slots",     value: activePlacements.length, icon: Layout, color: "text-status-success" },
            { label: "Tokens Spent",     value: `${totalSpent.toLocaleString()} ₮`, icon: Coins, color: "text-brand-gold" },
          ].map(stat => {
            const Icon = stat.icon;
            return (
              <div key={stat.label} className="bg-ink border border-white/8 rounded-xl p-4">
        ...
        {totalActive === 0 ? (
          <div className="bg-ink border border-white/8 rounded-xl py-16 text-center">
            ...
            <div className="flex items-center justify-center gap-3">
              <a href="/user-bid-keywords" className="px-4 py-2 bg-brand-yellow text-ink text-xs font-black rounded-lg hover:bg-brand-yellow-soft transition-colors">
                Bid Keywords
              </a>
              <a href="/user-page-placements" className="px-4 py-2 bg-ink-charcoal text-white text-xs font-bold rounded-lg hover:bg-ink-paragraph transition-colors border border-white/10">
                Book Slots
              </a>
            </div>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="bg-ink border border-white/8 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/8 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Target size={15} className="text-status-info" />
                    <span className="font-bold text-white text-sm">Keyword Bids ({activeBids.length})</span>
                  </div>
                  <a href="/user-bid-keywords" className="text-xs text-brand-yellow hover:underline">+ Add bid</a>
                </div>
                    ...
                    <tbody>
                      {activeBids.map(b => (
                        <tr key={b.bidId} className="border-t border-white/5 hover:bg-white/2">
                          ...
                          <td className="px-4 py-3 text-sm text-brand-yellow font-bold whitespace-nowrap">{b.bidAmount} ₮/day</td>
            ...
            {activePlacements.length > 0 && (
              <div className="bg-ink border border-white/8 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/8 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Layout size={15} className="text-status-success" />
                    <span className="font-bold text-white text-sm">Page Placements ({activePlacements.length})</span>
                  </div>
                  <a href="/user-page-placements" className="text-xs text-brand-yellow hover:underline">+ Book slot</a>
                </div>
                  ...
                  {activePlacements.map(p => (
                    <div key={p.placementId} className="flex items-center justify-between px-5 py-4">
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0 ml-4">
                        <span className="text-[11px] bg-status-success/15 text-status-success px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
            ...
            <div className="bg-ink border border-white/8 rounded-xl p-5">
              <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                <Coins size={15} className="text-brand-yellow" /> Token Summary
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[
                  { label: "Current Balance", value: `${tokenBalance.toLocaleString()} ₮`, color: "text-brand-yellow" },
                  { label: "Total Spent",     value: `${totalSpent.toLocaleString()} ₮`,   color: "text-white" },
                  { label: "Active Spend",    value: `${[...activeBids.map(b => b.totalCost), ...activePlacements.map(p => p.totalTokens)].reduce((s, v) => s + v, 0).toLocaleString()} ₮`, color: "text-status-info" },
                ].map(s => (
              ...
              <div className="mt-4 flex items-center justify-between">
                <a href="/user-recharge" className="text-xs text-brand-yellow hover:underline">View all transactions →</a>
                <a href="/user-buy" className="px-3 py-1.5 bg-brand-yellow/10 border border-brand-yellow/20 text-brand-yellow text-xs font-bold rounded-lg hover:bg-brand-yellow/20 transition-colors">
                  Buy Tokens
                </a>
              </div>
            </div>
