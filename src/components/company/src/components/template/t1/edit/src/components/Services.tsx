...
export default function Services({
  ...
  const uploadImageToAWS = async (file, index) => {
    ...
    try {
      setIsUploading(true);
            const __presignedUrl = await uploadCompanyImagePresigned(userId, `services-${index}-${Date.now()}`, file);
            ...
            <Card key={index} className="relative overflow-hidden w-[220px] min-h-[360px] rounded-2xl shadow-md border border-ink-light bg-surface-card flex flex-col">
              <div className="h-32 overflow-hidden relative rounded-t-2xl" style={{ background: 'linear-gradient(135deg, #fb923c 0%, #c2410c 100%)' }}>
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 pointer-events-none">
                  ...
                  <span style={{ color: 'white', fontSize: 10, fontWeight: 600 }}>{service.category || 'Service'}</span>
                </div>
                {service.image && (
                  <img
                    src={service.image}
