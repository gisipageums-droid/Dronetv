...
//   const tempHeroStateRef = useRef(tempHeroState);
...
//   useEffect(() => {
//     tempHeroStateRef.current = tempHeroState;
//     pendingImageFilesRef.current = pendingImageFiles;
//     isEditingRef.current = isEditing;
//   }, [tempHeroState, pendingImageFiles, isEditing]);
...
//       let updatedState = { ...tempHeroStateRef.current };
//       const uploadedImages = [];

//       // Upload hero1Image if pending
//       if (pendingImageFilesRef.current.hero1Image) {
//         const imageUrl = await uploadImageToAWS(pendingImageFilesRef.current.hero1Image, "hero1Image");
//         if (imageUrl) {
//           updatedState.hero1Image = imageUrl;
//           uploadedImages.push("hero1Image");
//         }
//       }

//       // Upload hero3Image if pending
//       if (pendingImageFilesRef.current.hero3Image) {
//         const imageUrl = await uploadImageToAWS(pendingImageFilesRef.current.hero3Image, "hero3Image");
//         if (imageUrl) {
//           updatedState.hero3Image = imageUrl;
//           uploadedImages.push("hero3Image");
//         }
//       }

//       // Upload customer images if pending
//       for (let i = 0; i < pendingImageFilesRef.current.customerImages.length; i++) {
//         if (pendingImageFilesRef.current.customerImages[i]) {
//           const imageUrl = await uploadImageToAWS(pendingImageFilesRef.current.customerImages[i], "customerImage", i);
//           if (imageUrl) {
//             updatedState.customerImages[i] = imageUrl;
//             uploadedImages.push(`customerImage-${i}`);
//           }
//         }
//       }
...
//       try {
//         const imageUrl = await uploadImageToAWS(file, cropField, cropIndex);
//         if (imageUrl) {
...
//           if (cropField === "hero1Image" || cropField === "hero3Image") {
//             setPendingImageFiles(prev => ({ ...prev, [cropField]: null }));
//           } else if (cropField === "customerImage" && cropIndex !== null) {
//             setPendingImageFiles(prev => {
//               const updated = [...prev.customerImages];
//               updated[cropIndex] = null;
//               return { ...prev, customerImages: updated };
//             });
//           }
...
//   // Memoized EditableText - fixed without auto-save interference
//   const EditableText = useMemo(() => {
//     return ({
//       value,
//       field,
...
export default function EditableHero({
  ...
  useEffect(() => {
    if (isEditing && hasUnsavedChanges) {
      ...
      // Set new timeout for auto-save (3 seconds as requested)
  ...
  // FIXED: applyCrop now uploads immediately
  // const applyCrop = async () => {
  //   try {
