...
export default function EditableHero({
  ...
  const uploadImageToAWS = async (file, fieldName, index = null) => {
    ...
    try {
      setIsUploading(true);
            const __presignedUrl = await uploadCompanyImagePresigned(userId, `${fieldName}${index !== null ? `-${index}` : ''}-${Date.now()}`, file);
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full ">
          {!isEditing ? (
            ...
            <EditableText
              ...
              field="badgeText"
              className="text-lg sm:text-xl md:text-2xl text-left text-[#F8C400] relative z-20 mb-8 inline-block max-w-[530px]"
