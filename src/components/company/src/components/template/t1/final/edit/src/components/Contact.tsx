...
export default function EditableContact({
  ...
  useEffect(() => {
    if (content) {
      const mergedData = { ...defaultContent, ...content };
  ...
  const handleSave = async () => {
    try {
      ...
      if (onStateChange) onStateChange(tempData);
    } catch (error) {
  ...
  const addBusinessHour = () => {
    setTempData((prev) => ({
      ...prev,
      businessHours: [...prev.businessHours, "New business hour"],
    }));
  };

  const removeBusinessHour = (index) => {
    if (tempData.businessHours.length > 1) {
      setTempData((prev) => ({
        ...prev,
