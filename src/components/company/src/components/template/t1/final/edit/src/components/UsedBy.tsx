...
//   // Enhanced crop modal state (same as Header.tsx and Hero.tsx)
...
//   // Enhanced cropper functions (same as Header.tsx and Hero.tsx)
...
//   return (
//     <>
//       {/* Enhanced Crop Modal (same as Header.tsx and Hero.tsx) */}
//       {cropModalOpen && (
//         <motion.div
//           initial={{ opacity: 0 }}
//           animate={{ opacity: 1 }}
...
export default function EditableUsedBy({
  ...
  const autoSave = useCallback(async () => {
    if (!isEditing || isPublished) return;

    try {
      setIsSaving(true);

      // Update state with any changes
      setContentState(tempContentState);
  ...
  }, [isEditing, tempContentState, isPublished]);
  ...
  }, [isEditing, autoSave, isPublished]);
  ...
  const updateCompanyName = useCallback(
    (companyId, newName) => {
      ...
      scheduleAutoSave(); // Trigger auto-save on company name update
    },
    [isPublished, scheduleAutoSave]
  );
  ...
  const addCompany = useCallback(() => {
    ...
    scheduleAutoSave(); // Trigger auto-save for new company
  }, [isPublished, scheduleAutoSave]);
  ...
  const removeCompany = useCallback(
    (companyId) => {
      ...
      scheduleAutoSave(); // Trigger auto-save for company removal
    },
    [isPublished, scheduleAutoSave]
  );
  ...
  return (
    <>
      ...
      <section ref={sectionRef} className="py-16 bg-white relative">
        ...
        {isEditing && !isPublished && (
          <motion.div
            className="absolute top-4 left-4 flex items-center gap-2 text-xs text-gray-500 z-30"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            {isSaving ? (
