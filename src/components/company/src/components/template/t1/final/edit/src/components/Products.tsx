...
//       return {
//         // Map directly from productData root level (not from heading object)
...
//                   onChange={(val) =>
//                     setTempContent({ ...tempContent, trustText: val })
//                   }
...
//   const uploadImageToS3 = async (file: File, productId: number): Promise<string | null> => {
//     try {
//       if (!userId || !publishedId || !templateSelection) {
...
//       pendingUploadsRef.current.set(cropIndex, { file, previewUrl });
...
//       try {
//         const s3Url = await uploadImageToS3(file, cropIndex);
//         if (s3Url) {
...
//           pendingUploadsRef.current.delete(cropIndex);
...
//       const uploadPromises = Array.from(pendingUploadsRef.current.entries())
//         .map(async ([productId, { file }]) => {
//           try {
//             const s3Url = await uploadImageToS3(file, productId);
//             if (s3Url) {
//               return { productId, s3Url };
//             }
//           } catch (error) {
...
//       // Update content with S3 URLs
//       let updatedContent = { ...tempContent };
      
//       uploadResults.forEach((result) => {
//         if (result.status === "fulfilled" && result.value) {
//           const { productId, s3Url } = result.value;
//           updatedContent.products = updatedContent.products.map((product) =>
//             product.id === productId
//               ? { ...product, image: s3Url }
//               : product
