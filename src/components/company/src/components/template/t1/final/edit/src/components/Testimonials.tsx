...
export default function EditableTestimonials({
  ...
  const autoSave = useCallback(async () => {
    ...
    try {
      ...
      let updatedData: TestimonialsContent = { ...tempData };
      const uploadedImages: Record<number, string> = {};

      for (const [testimonialIdStr, file] of Object.entries(pendingImages)) {
        const testimonialId = parseInt(testimonialIdStr, 10);
        const imageUrl = await uploadImageToAWS(file, testimonialId);
        
        if (imageUrl) {
          uploadedImages[testimonialId] = imageUrl;
          // Update the testimonial image in our local copy
          updatedData.testimonials = updatedData.testimonials.map(
            (testimonial, index) =>
              index === testimonialId
  ...
  }, [isEditing, tempData, pendingImages, userId, publishedId, templateSelection]);
  ...
  const applyCrop = async () => {
    try {
      ...
      if (imageUrl) {
        ...
        setTestimonialsData((prev: TestimonialsContent) => ({
          ...prev,
          testimonials: prev.testimonials.map((testimonial, idx) =>
            idx === cropIndex
              ? { ...testimonial, image: imageUrl }
  ...
  const updateHeadlineField = (
    ...
    scheduleAutoSave(); // Trigger auto-save on headline update
  };
  ...
  const updateTestimonialField = (
    ...
    scheduleAutoSave(); // Trigger auto-save on testimonial field update
  };
  ...
  const addTestimonial = () => {
    ...
    scheduleAutoSave(); // Trigger auto-save for new testimonial
  };
  ...
  const deleteTestimonial = (index: number) => {
    ...
    scheduleAutoSave(); // Trigger auto-save for testimonial deletion
  };
