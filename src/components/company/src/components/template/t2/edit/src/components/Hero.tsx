...
import { ArrowRight, Play, CheckCircle, X, RotateCw, ZoomIn, ZoomOut } from "lucide-react";
...
export default function Hero({ heroData, onStateChange, userId, publishedId, templateSelection }) {
  ...
  const [heroState, setHeroState] = useState({
    ...
    heroImage: heroData?.heroImage || "https://images.unsplash.com/photo-1698047682129-c3e217ac08b7?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtb2Rlcm4lMjBidXNpbmVzcyUyMHRlYW0lMjBvZmZpY2V8ZW58MXx8fHwxNzU1NjE4MzQ4fDA&ixlib=rb-4.1.0&q=80&w=1080",
    hero3Image: heroData?.hero3Image || "https://images.unsplash.com/photo-1552664730-d307ca884978?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxidXNpbmVzcyUyMG1lZXRpbmd8ZW58MXx8fHwxNzU1NjE5MDEzfDA&ixlib=rb-4.1.0&q=80&w=400"
  });
    ...
    try {
      ...
      if (pendingImageFile) {
        ...
        if (uploadResponse.ok) {
          ...
          toast.error(`Main image upload failed: ${errorData.message || 'Unknown error'}`);
          return;
        }
      }
      ...
      if (pendingSmallImageFile) {
        ...
        if (uploadResponse.ok) {
          ...
          toast.error(`Small image upload failed: ${errorData.message || 'Unknown error'}`);
          return;
        }
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div className="relative" initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.5 }}>
              ...
              <motion.div className="absolute -bottom-6 -left-6 bg-card p-2 rounded-xl shadow-lg border border-border" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5 }}>
