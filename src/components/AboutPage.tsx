...
const AboutPage = () => {
  ...
  return (
    <div className="pt-[104px] min-h-screen bg-surface-main">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div className="space-y-5">
              <div className="bg-ink-offwhite rounded-xl border border-ink-light p-7 hover:shadow-md transition-shadow">
                <div className="flex items-center mb-4">
                  <div className="bg-brand-yellow rounded-full p-3 mr-4 flex-shrink-0">
                    <Target className="h-6 w-6 text-ink" />
                  </div>
                  <h2 className="text-xl font-extrabold text-ink">Our Mission</h2>
                </div>
              ...
              <div className="bg-ink-offwhite rounded-xl border border-ink-light p-7 hover:shadow-md transition-shadow">
                <div className="flex items-center mb-4">
                  ...
                  <h2 className="text-xl font-extrabold text-ink">Our Vision</h2>
                </div>
                <p className="text-sm text-ink-paragraph leading-relaxed">
            ...
            <div className="bg-ink-offwhite rounded-xl border border-ink-light p-8">
              ...
              <div className="text-center">
                <h3 className="text-lg font-bold text-ink mb-3">Shaping Tomorrow's Technology</h3>
                <p className="text-sm text-ink-caption leading-relaxed">Through comprehensive education and industry partnerships, we're building the foundation for the next generation of drone innovations.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Founder's Message */}
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <h2 className="text-lg font-bold text-ink flex items-center gap-3 mb-8 after:flex-1 after:h-0.5 after:bg-ink-light after:content-['']">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
            <div className="lg:col-span-1 flex justify-center">
              <div className="relative">
                ...
                <div className="absolute -bottom-3 -right-3 bg-brand-yellow rounded-full p-3 shadow-md">
                  <Heart className="h-5 w-5 text-ink" />
                </div>
              </div>
            </div>
            <div className="lg:col-span-2">
              <div className="space-y-3 text-sm text-ink-paragraph leading-relaxed">
                ...
                <p className="font-bold text-ink">We're not just documenting the future — we're helping build it.</p>
              </div>
              <div className="mt-6">
                <div className="text-base font-bold text-ink">Dev R</div>
                <div className="text-sm text-ink-caption">Founder &amp; CEO, Drone TV</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-surface-card border-t border-ink-light">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <h2 className="text-lg font-bold text-ink flex items-center gap-3 mb-2 after:flex-1 after:h-0.5 after:bg-ink-light after:content-['']">
            Meet Our Team
          </h2>
          <p className="text-sm text-ink-caption mb-8">The passionate individuals behind Drone TV's mission to democratize drone technology education.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {teamMembers.map((member) => (
              <div
                key={member.id}
                className="group bg-ink-offwhite rounded-xl border border-ink-light overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                onMouseEnter={() => setHoveredTeamMember(member.id)}
                onMouseLeave={() => setHoveredTeamMember(null)}
              >
                <div className="relative overflow-hidden bg-ink-light h-64 flex items-center justify-center">
                  <img
                    src={member.image}
                    alt={member.name}
                ...
                <div className="p-5">
                  ...
                  <div className="text-xs font-semibold text-brand-gold mb-2">{member.role}</div>
                  <p className="text-xs text-ink-caption leading-relaxed">{member.bio}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12" ref={timelineRef}>
        ...
        <p className="text-sm text-ink-caption mb-10">Key milestones in Drone TV's evolution from startup to industry leader.</p>

        <div className="relative">
          <div className="hidden sm:block absolute left-1/2 transform -translate-x-1/2 w-0.5 h-full bg-ink-light rounded-full" />
              ...
              return (
                <div key={index} className="timeline-card">
                  ...
                  <div className="sm:hidden">
                    <div
                      className={`bg-surface-card rounded-xl border border-ink-light shadow-sm p-5 transition-all duration-500 ${isVisible ? 'animate-slide-in-left' : 'opacity-0'}`}
                      style={{ animationDelay: `${index * 0.2}s`, animationFillMode: 'forwards' }}
                    >
                      <div className="flex items-center gap-3 mb-3">
                        <div className="w-10 h-10 flex-shrink-0 bg-surface-card rounded-full border-2 border-brand-yellow flex items-center justify-center shadow-sm">
                          <IconComponent className="h-5 w-5 text-ink" />
                        </div>
                        <span className="bg-ink text-brand-yellow px-3 py-1 rounded-full text-xs font-bold">{event.year}</span>
                      </div>
                      <h3 className="text-sm font-bold text-ink mb-2">{event.title}</h3>
                      <p className="text-xs text-ink-caption leading-relaxed">{event.description}</p>
                    </div>
                  </div>
                  ...
                  <div className="hidden sm:grid items-center" style={{ gridTemplateColumns: '1fr 80px 1fr' }}>
                    <div className="pr-8">
                      {isEven && (
                        <div
                          className={`bg-surface-card rounded-xl border border-ink-light shadow-sm p-6 transition-all duration-500 ${isVisible ? 'animate-slide-in-left' : 'opacity-0'}`}
                          style={{ animationDelay: `${index * 0.2}s`, animationFillMode: 'forwards' }}
                        >
                          <span className="inline-block bg-ink text-brand-yellow px-3 py-1 rounded-full text-xs font-bold mb-3">{event.year}</span>
                          <h3 className="text-sm font-bold text-ink mb-2">{event.title}</h3>
                          <p className="text-xs text-ink-caption leading-relaxed">{event.description}</p>
                        </div>
                      )}
                    ...
                    <div className="flex items-center justify-center relative z-10">
                      <div className="w-14 h-14 bg-surface-card rounded-full border-2 border-brand-yellow flex items-center justify-center shadow-md">
                    ...
                    <div className="pl-8">
                      {!isEven && (
                        <div
                          className={`bg-surface-card rounded-xl border border-ink-light shadow-sm p-6 transition-all duration-500 ${isVisible ? 'animate-slide-in-right' : 'opacity-0'}`}
                          style={{ animationDelay: `${index * 0.2}s`, animationFillMode: 'forwards' }}
                        >
                          <span className="inline-block bg-ink text-brand-yellow px-3 py-1 rounded-full text-xs font-bold mb-3">{event.year}</span>
                          <h3 className="text-sm font-bold text-ink mb-2">{event.title}</h3>
      ...
      <div className="bg-ink">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="mailto:bd@dronetv.in" className="bg-brand-yellow text-ink px-7 py-3 rounded-xl font-bold text-sm hover:bg-brand-yellow-soft transition-all flex items-center gap-2 justify-center">
              Get In Touch <ArrowRight className="h-4 w-4" />
            </a>
            <button onClick={() => navigate('/partner')} className="border border-brand-yellow text-brand-yellow px-7 py-3 rounded-xl font-bold text-sm hover:bg-brand-yellow hover:text-ink transition-all">
              Partner With Us
            </button>
          </div>
        </div>
