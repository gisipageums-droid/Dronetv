          ...
          {categoryOptions.map(({ value, icon: Icon, description }) => (
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <Icon className={`w-12 h-12 mb-4 ${formData.companyCategory.includes(value) ? 'text-status-info' : 'text-ink-caption'
                }`} />
              <h3 className={`text-xl font-bold mb-2 ${formData.companyCategory.includes(value) ? 'text-status-info' : 'text-ink-paragraph'
                }`}>
                {value}
              </h3>
              <p className={`text-sm text-center ${formData.companyCategory.includes(value) ? 'text-status-info' : 'text-ink-caption'
                }`}>
                {description}
              </p>
            </label>
