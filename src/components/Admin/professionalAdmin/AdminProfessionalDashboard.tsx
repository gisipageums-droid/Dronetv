import { Award, CheckCircle, ChevronDown, Eye, FileText, MapPin, Menu, Search, Trash2, User, X, XCircle, Clock } from "lucide-react";
            ...
            <div className='text-center max-w-3xl relative w-full'>
                ...
                <div className='relative z-10'>
                    ...
                    <p className='text-base md:text-lg text-blue-700 mb-6 md:mb-10 max-w-xl mx-auto font-light'>
                        Review and manage all professional profiles, credentials, and approvals.
                    </p>
            ...
            <div className='p-4 md:p-6 lg:p-8'>
                    ...
                    <div className='flex items-center gap-3 md:gap-4'>
                        ...
                        <div className="max-w-[calc(100%-60px)] md:max-w-none">
                            ...
                            <div className='flex items-center text-gray-600 mt-1'>
                                ...
                                <span className='text-xs md:text-sm'>{professional.location || 'Location not specified'}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className='mb-4 md:mb-6'>
                    <div className='flex flex-wrap gap-1 md:gap-2'>
                        {(professional.categories && professional.categories.length > 0 ? professional.categories : ['General']).map((category: string, index: number) => (
                            <span
                                key={index}
                    ...
                    <div className='flex items-center gap-3 md:gap-6'>
                        <div className='flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-1 md:px-4 md:py-2'>
                            <span className='font-bold text-purple-600 text-xs md:text-sm'>
                                {professional.publishedDate ? formatDate(professional.publishedDate) : 'Not published'}
                            </span>
                            <span className='text-xs text-gray-600 hidden md:block'>Created</span>
                        </div>
                    </div>
                ...
                <div className='mt-3 md:mt-4 pt-3 md:pt-4 border-t border-gray-100'>
                    <div className='flex justify-between items-center text-xs text-gray-400'>
                        <span className="truncate mr-2">ID: {professional.professionalId || 'No ID'}</span>
...
const RecentProfessionalsSection: React.FC<{
    ...
    onDelete: (professionalId: string) => void;
    disabled?: boolean;
}> = ({ recentProfessionals, onCredentials, onPreview, onApprove, onReject, onDelete, disabled }) => {
            ...
            {professionals.length > 0 ? (
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6'>
                    {professionals.map((professional: Professional, index: number) => (
                        <div key={professional.professionalId || index} className='animate-fadeIn'>
                            <ProfessionalCard
                                professional={professional}
                                onCredentials={onCredentials}
                                onPreview={onPreview}
                ...
                <>
                    ...
                    {searchTerm || categoryFilter !== "All Categories" || sortBy !== "Sort by Name" ? (
                        ...
                        <div className='text-center py-12 md:py-16'>
                            <div className='text-6xl mb-4'>🔍</div>
                            <p className='text-xl text-gray-700 mb-2'>No professionals match your filters</p>
                        ...
                        <div className='text-center py-12 md:py-16'>
                            <div className='text-6xl mb-4'>👨‍💼</div>
                            <p className='text-xl text-gray-700 mb-2'>No professionals found</p>
                            <p className='text-gray-500 mb-6'>
...
const AdminProfessionalDashboard: React.FC = () => {
    ...
    const handlePreview = async (professionalId: string): Promise<void> => {
        try {
            ...
            if (professional.templateSelection === "template-1") {
                navigate(`/admin/professionals/preview/1/${professionalId}/${professional.userId}`);
            } else if (professional.templateSelection === "template-2") {
                navigate(`/admin/professionals/preview/2/${professionalId}/${professional.userId}`);
            }
        } catch (error) {
    ...
    const handleApprove = async (professionalId: string): Promise<void> => {
        try {
            setIsMutating(true);
            const professional = professionals.find(p => p.professionalId === professionalId);
            const result = await apiService.approveProfessional(professionalId, professional.userId);
    ...
    const handleReject = async (professionalId: string): Promise<void> => {
        try {
            setIsMutating(true);
              const professional = professionals.find(p => p.professionalId === professionalId);
            const result = await apiService.rejectProfessional(professionalId, professional.userId);
    ...
    const handleDelete = async (professionalId: string): Promise<void> => {
        if (!window.confirm("Are you sure you want to delete this professional? This action cannot be undone.")) {
            return;
        }
    ...
    const categories: string[] = [
        "All Categories",
        ...Array.from(new Set(professionals.flatMap((p: Professional) => p.categories || []))).sort(),
    ];
    ...
    const filteredProfessionals = professionals.filter((professional: Professional) => {
        const matchesSearch = !searchTerm ||
            (professional.professionalName &&
                professional.professionalName.toLowerCase().includes(searchTerm.toLowerCase())) ||
        ...
        const matchesCategory =
            ...
            (professional.categories && professional.categories.includes(categoryFilter));

        return matchesSearch && matchesCategory;
    });

    // Sort professionals
    const sortedProfessionals = [...filteredProfessionals].sort((a: Professional, b: Professional) => {
        switch (sortBy) {
    ...
    return (
        <div className='min-h-screen bg-blue-100'>
            ...
            <ProfessionalCredentialsModal
                ...
                professional={professionals.find(p => p.professionalId === credentialsModal.data?.professionalId) || null}
            />
