  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Services Section */}
        {/* <div>Main categories: {formData.mainCategories} </div>
        <p>
  Selected Subcategories : 
  {formData.subCategories
    ? Object.values(formData.subCategories)
    .flat()
    .join(', ')
...
<div>
  <h4>Selected Sub-Subcategories:</h4>
  {formData.subSubCategories && Object.entries(formData.subSubCategories).map(([subCategory, subs]) => (
    <div key={subCategory}>
      <strong>{subCategory}:</strong> {subs.join(', ')}
    </div>
  ))}
</div>
    ...
    <div>Custom Categories:{formData.otherMainCategories}</div>
 */}
