...
ARG VITE_BACKEND_URL_RFQ=""
ARG VITE_BACKEND_URL_PAYMENT=""
ARG VITE_BACKEND_URL_ADMIN=""
ARG VITE_BACKEND_URL_JOB_APPLICATIONS=""
# .env.production/.env.development are gitignored, so this value never
# reaches a Jenkins-built image via `COPY . .` - without it, GST/CIN/
# DigiLocker verification's Lambda fallback silently posts to
# "<origin>/undefined" (405) whenever our own backend can't serve the
# request (e.g. a revoked Surepass token).
ARG VITE_SUREPASS_PROXY_URL=""
...
ENV VITE_BACKEND_URL_AUTH=$VITE_BACKEND_URL_AUTH \
    ...
    VITE_BACKEND_URL_RFQ=$VITE_BACKEND_URL_RFQ \
    ...
    VITE_SUREPASS_PROXY_URL=$VITE_SUREPASS_PROXY_URL
